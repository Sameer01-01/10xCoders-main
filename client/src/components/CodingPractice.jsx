    ...
    setTimeout(() => {
      try {
        ...
        if (language === 'c' || language === 'cpp') {
          ...
          if (!code.includes('main')) {
            setError('Error: No main function found');
                     ...
                     (language === 'cpp' && !code.includes('#include') && !code.includes('using namespace'))) {
        ...
        if (selectedQuestion) {
          ...
          if (question && !question.solved && code.includes(question.solution)) {
    ...
    <div className={`flex flex-col h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'}`}>
      ...
      <div className={`flex justify-between items-center p-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-200'}`}>
        <div className="flex items-center space-x-4">
          ...
          <select 
            className={`p-1 rounded ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'}`}
            value={language}
          ...
          <button 
            onClick={toggleTheme}
            className={`px-3 py-1 rounded ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-white hover:bg-gray-300'}`}
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showQuestions && (
          <div className={`w-64 p-4 overflow-y-auto ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-r ${theme === 'dark' ? 'border-gray-700' : 'border-gray-300'}`}>
        ...
        {/* Editor */}
        <div className={`${showQuestions ? 'w-1/2' : 'w-1/2'} p-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-r ${theme === 'dark' ? 'border-gray-700' : 'border-gray-300'}`}>
          ...
          <textarea
            ref={editorRef}
            className={`w-full ${selectedQuestion ? 'h-3/4' : 'h-full'} p-2 font-mono text-sm resize-none rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        {/* Output */}
        <div className={`${showQuestions ? 'w-1/3' : 'w-1/2'} p-4 ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}>
          <div className="flex justify-between mb-2">
